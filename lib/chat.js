...
function chatHandler (socket) {
  ...
  socket.on('io:name', function (user) {
	  var pass = JSON.stringify({u:user.name,i:user.avatar}); 
    pub.HSET("people", socket.client.conn.id, pass);
     console.log(socket.client.conn.id + " > " + user.name + ' joined chat! and url avatar: '+user.avatar);
    pub.publish("chat:people:new", pass);
  });

  socket.on('io:message', function (msg) {
    console.log("msg:", msg);
    msg = sanitise(msg);
    pub.HGET("people", socket.client.conn.id, function (e,pass) {
		variable = JSON.parse(pass);
       console.log("io:message received: " + msg + " | from: " +variable.u);
      var str = JSON.stringify({ // store each message as a JSON object
	m: msg,
	t: new Date().getTime(),
	n: variable.u,
	i: variable.i
	//i: "https://cdn2.iconfinder.com/data/icons/faceavatars/PNG/D04.png"
      });
      console.log(str);
